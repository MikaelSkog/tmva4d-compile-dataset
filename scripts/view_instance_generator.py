...
class ViewInstancesGenerator():
    def __init__(self, point_cloud_file_path, pcd_stats, img_resolution):
        img_width  = 640
        img_height = 512
        ...
        def update_matrices(point_idx):
            ...
            azimuth_bin_idx = round(azimuth_values[point_idx] * (AZIMUTH_SIZE-1) / (img_width-1))
            elevation_bin_idx = round(elevation_values[point_idx] * (ELEVATION_SIZE-1) / (img_height-1))
